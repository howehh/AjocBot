...
function handleTwitchLive(channel) {
   axios.get('https://api.twitch.tv/helix/streams?user_id=' + channel.id, {
      headers: {
         "Client-ID": config.twitchKey
      }
   }).then(function(response) {
      if (response.data.data.length !== 0 && !channel.live) {
         channel.live = true;
         if (!state.firstAlert) {
      ...
      } else if (channel.live && response.data.data.length === 0) {
         channel.live = false;
      }
