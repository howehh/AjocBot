      ...
      axios.get('https://api.twitch.tv/helix/streams' + twitchNames, {
         ...
         for (let i = 0; i < twitchObjs.length; i++) {
            ...
            for (let j = 0; j < response.data.data.length; j++) {
               ...
               // if we found the channel (it's live) and it's currently not set to live,
               // then queue it up, alert, and set to live
               if (currResponseObj.user_id === channel.id && !channel.live) {
