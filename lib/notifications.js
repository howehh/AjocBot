...
let channels = []; // array of youtube/twitch channels (name, tw/yt tag, and additional tag
let newestUploads = new Map(); // maps youtube channel to video ID of their newest upload 
...
function newerUpload(channelName, videoId, time) {
   ...
   // verifies that this newest upload has been uploaded within 30 mins before now
   if (videoId !== newestUploads.get(channelName) && (Date.now() - time < 600000)) {
...
function incrementFirstAlertCount() {
   if (++firstAlertCount === channels.length) {
      firstAlert = false;
   }
}
...
function handleTwitchLive(channel) {
   ...
   .then(function(response) {
      ...
      if (firstAlert) incrementFirstAlertCount();
   });
}
