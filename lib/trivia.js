...
const trivia = {
   playGame: false,
   questionNum: 0,
   currentQuestion: "Do you love Alizee?",
   currentAnswers: ["yes", "oui"],
   questionsLength: 0,
   CORRECT_PTS: 80,
   SKIP_DEDUCT: 60
};
...
.then(function(response) {
   if (response.data.files.trivianum !== undefined) {
      let num = parseInt(response.data.files.trivianum.content);
      if (isNaN(num)) {
...
function getQuestion() {
   ...
   .then(function(response) {
      if (response.data.questions !== undefined) { 
         ...
         trivia.questionNum = trivia.questionNum % response.data.questions.length;
         ...
         throw new Error("Please define a trivia paste in config.json with the valid format");
      }
   });
}
...
function checkForAnswer(data) {
   ...
   if (msgIncludesAnswer(data.msg)) {
      trivia.questionNum = (trivia.questionNum + 1) % trivia.questionsLength;
