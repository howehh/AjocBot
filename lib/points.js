...
function checkIfDuplicate(name, aliases) {
   ...
   if (aliases !== undefined) {
      ...
      for (let i = 0; i < aliases.length; i++) {
         if (userData.points.has(aliases[i].toLowerCase())) {
            return true;
         }
      }
   }
...
bot.addUserlistEvent(function(userlist) {
   ...
   .then(function(response) {
      ...
      for (let i = 0; i < json.users.length; i++) {
         userData.points.set(json.users[i].name, json.users[i].points);
      }
...
function adjustPoints(currentUser, pointAdjustment) {
   ...
   if (!userData.points.has(currentUser)) {
      userData.points.set(currentUser, 0);
   }
   let newScore = userData.points.get(currentUser) + pointAdjustment;
   newScore = (newScore < 0) ? 0 : newScore;
   userData.points.set(currentUser, newScore);
}
...
function pointEligible(username) {
   ...
   return ((userData.points.has(username) && userData.points.get(username) > 0) || 
...
const publicFunctions = {
   ...
   points: function(data) { // Shows a user's number of points and their scoreboard position
      if (data.msg.startsWith("!points")) {
         ...
         if (!userData.points.has(username)) {
            username = data.username.toLowerCase();
         }
         if (userData.points.has(username) && userData.points.get(username) > 0) {
            result = username + " has " + userData.points.get(username) + " points";
      ...
      if (data.msg.startsWith("!give")) {
         ...
         if (tokens.length === 3) {
            ...
            if (!isNaN(amount) && amount > 0 && pointEligible(target)) {
               if (userData.points.has(username) && userData.points.get(username) >= amount) {
   ...
   setPoints: function(data) {
      if (data.msg.startsWith("!setpoints") && bot.isAdmin(data.username)) {
         ...
         if (tokens.length === 3) {
            ...
            const amount = parseInt(tokens[2]);
            if (!isNaN(amount) && amount >= 0 && pointEligible(target)) {
               userData.points.set(target, amount);
