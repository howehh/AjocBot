...
const publicFunctions = {
   ...
   startRaffle: function(data) {
      if (data.msg.indexOf("!raffle ") != -1 && bot.isAdmin(data)) {
         const tokens = data.msg.trim().split(/\s+/g);
   ...
   setPoints: function(data) {
      if (data.msg.startsWith("!setpoints") && bot.isAdmin(data)) {
         const tokens = data.msg.trim().split(/\s+/g);
         if (tokens.length === 3) {
            const username = data.username.toLowerCase();
            const target = tokens[1].toLowerCase();
   ...
   endRaffle: function(data) {
      if (data.msg.indexOf("!endraffle") !== -1 && bot.isAdmin(data) && raffle.timeout !== null) {
         clearRaffle();
