...
Game.prototype.summary = function() {
   if (!this.gameStarted) 
      throw "startGame should be completed before requesting summary";
...
Game.prototype.startGame = function() {
   ...
   let firstDealerCard = this.deck.drawCard();
   let secondDealerCard = this.deck.drawCard();
   this.dealerCards.push(firstDealerCard, secondDealerCard);
   
   // Determine initial points for the dealer and update accordingly
   if (firstDealerCard.name === "A" && secondDealerCard.name === "A") {
      firstDealerCard.value = 1;
      secondDealerCard.value = 11;
   } else if (firstDealerCard.name === "A") {
      firstDealerCard.value = (secondDealerCard.value === 6) ? 1 : 11;
   } else if (secondDealerCard.name === "A") {
      secondDealerCard.value = (firstDealerCard.value === 6) ? 1 : 11;
   }
...
Game.prototype.hit = function(card) {
   if (!this.gameStarted) {
      throw "startGame should be completed before doing a hit";
   }
...
Game.prototype.dealerFinish = function(initialMessage) {
   if (!this.gameStarted) {
      throw "startGame should be completed before a game is finished";
   }
