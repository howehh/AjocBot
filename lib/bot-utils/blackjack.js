...
class BlackjackGame {
   ...
   startGame() {
      ...
      let message = this.playerName + ": You got a " + firstCard.getName() + firstCard.getSuit() +
                    " and a " + secondCard.getName() + secondCard.getSuit() + "./br//br/";
                    
      if (this.playerHigh === 21) {
         this.dealerFinish(message);
         return false;
      } else {
         bot.chat(message + this.summary());
         return true;
      }
   }
   ...
   hit(card) {
      ...
      if (this.playerLow < 21) { // continue playing
         message += this.summary();
         bot.chat(message);
         return true;
      } else { // player's turns are over
         message += " You now have a total of " + this.playerLow + ".";
         this.dealerFinish(message);
         return false;
      }
   }
