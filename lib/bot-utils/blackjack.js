...
class BlackjackGame {
   ...
   summary() {
      ...
      let message = `My face-up card is ${this.dealerCards[1]}.`;
      if (this.playerCards.length === 0) {
         message += " Your hand is currently empty.";
      } else {
         message += ` Your hand is ${this.playerCards[0]}`;
   ...
   startGame() {
      ...
      let message = `${this.playerName}: You got a ${firstCard} and a ${secondCard}`;
   ...
   hit(card) {
      ...
      let message = `${this.playerName}: you got a ${card}./br//br/`;
   ...
   surrender() {
      ...
      let message = this.getPlayerName() + ": you have surrendered.";
      endGame.call(this, message, -0.5);
   }
}
...
function endGame(initialMessage, pointMultiplier) {
   let message = `${initialMessage} I drew the ${this.dealerCards[0]}`;
   for (let i = 1; i < this.dealerCards.length; i++) {
      message += `, ${this.dealerCards[i]}`;
   }
