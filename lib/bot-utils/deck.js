...
class Card {
   ...
   // Changes value of the card - only allowed for aces (change to 1 or 11)
   setValue(val) {
      if (!this.isAce()) {
         throw new Error("You may only change ace values");
      } else if (val !== 11 && val !== 1) {
         throw new Error("Ace values may only be 11 or 1");
      }
      this.#value = val;
   }
}
