...
// ****************************** Chat Msg Handler ******************************* 
// On a chat message event, data is sent to each chatMsgEvent function
// *******************************************************************************
bot.socket().on("connect", () => setTimeout(listenForMessages, 2000));
bot.socket().on("disconnect", () => bot.socket().off("chatMsg"));
...
function listenForMessages() {   
   bot.socket().on("chatMsg", data => {
      ...
      data["tokens"] = data.msg.trim().split(/\s+/g);
