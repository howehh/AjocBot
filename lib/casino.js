...
module.exports = {
   ...
   playBlackjack: function(data) {
      if (data.username in games) {
         ...
         if (game.drewAce()) { // wait for them to call a value for ace
            let val;
            if (data.msg.trim() === "1") {
               val = 1;
            ...
            if (val !== undefined) {
               activity(data.username);
               let currAce = game.getCurrentAce();
               currAce.setValue(val);
               if (!game.hit(currAce)) {
            ...
            if (card.isAce()) {
               game.setCurrentAce(card);
               bot.chat(data.username + ": you got an A" + card.getSuit() +
