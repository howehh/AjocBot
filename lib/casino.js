...
function activity(username) {
   ...
   games[username].timeout = setTimeout(function() {
      let ptAdjust = Math.round(-0.5 * games[username].game.getBet());
      bot.chat(username + ": You haven't hit or stayed for 3 minutes. The game has been cancelled" +
               " and you have lost half your deposit (" + Math.abs(ptAdjust) + " pts) AlizeeSleeper");
