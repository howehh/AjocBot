...
function gambleCooldown(username) {
   gamblePlayers[username] = setTimeout(function() {
      gamblePlayers[username] = null;
   }, 1000);
}
...
function activity(username) {
   ...
   games[username].timeout = setTimeout(function() {
      ...
      game.surrender();
      let ptAdjust = Math.floor(game.getPointMultiplier() * game.getBet());
...
function finishGame(game, initialMessage) {
   ...
   let adjust = Math.floor(game.getBet() * pointMultiplier);
   
   if (pointMultiplier > 0) {
      let pts = points.getPoints(game.getPlayerName());
      if (game.getBet() > pts) {
         adjust = Math.floor(pts * pointMultiplier);
         bot.chat(`${message} Since you now have fewer ${points.CURRENCY} than your initial bet`
                  + ` you won only ${adjust} ${points.CURRENCY}. AlizeeOui`);
      } else {
...
const chatEvents = {
   ...
   startBlackjack: function(data) { // Chat command - Starts a game of blackjack
      if (data.tokens[0] === "!blackjack" && data.tokens.length > 1) {
         ...
         if (points.isBusy(data.username)) {
            bot.chat(`${data.username}: you are currently busy and can't play blackjack AlizeeWeird`);
            return
         }
         ...
         if (isNaN(amount) || amount < 1 || amount > 500) {
            ...
            points.setBusy(data.username, true);     
      ...
      if (data.username in games) {
         ...
         if (data.msg.trim() === "!stay") {
            ...
            let message = `${data.username}: you have surrendered. ` +
                          `Your next card would have been ${game.drawNewCard()}.`;
            finishGame(game, message);
         }
      }             
   },
   ...
   gamble: function(data) {
      if (data.tokens[0] === "!gamble") {
         if (data.username in gamblePlayers && gamblePlayers[data.username] !== null) {
            return;
         }
         const maxBet = 200;
