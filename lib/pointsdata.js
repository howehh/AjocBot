...
const pointJson = require('./../data/points');
...
function addUserStatus(name, duplicate) {
   ...
   if (name !== bot.BOTNAME.toLowerCase()) {
      ...
      userData.statuses.set(name,  {"dupl": duplicate, "stopwatch": null});  
...
// Gets the points of each user from a file, adds those users/points to the map of points. 
...
bot.addUserlistEvent(function(userlist) {
   for (let i = 0; pointJson.users !== undefined && i < pointJson.users.length; i++) {
      setPoints(pointJson.users[i].name, pointJson.users[i].points);
   }
