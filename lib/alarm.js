...
bot.firestoreDb().collection('botdata').doc('jsons').get().then(doc => {
   if (!doc.exists) {
      ...
      if (doc.data().alarms !== undefined) {
         let json = JSON.parse(doc.data().alarms);
      ...
      alarmData.alarmsFromCloudSet = true;
   }
});
...
shutdown.addShutdownData(function() {
   let data = {
      filename: "alarms",
      shutdownCheck: alarmData.alarmsFromCloudSet,
