...
// Takes a username, milliseconds, and the equivalent days/hours/mins/seconds. 
// Sets the given user's alarm (timeout) and when the time has elapsed, starts a spam (interval)
// The spam stops after a certain duration (timeout)
function setAlarmHelper(username, ms, days, hours, mins, seconds) {
   alarmUsers.get(username).currentAlarm = setTimeout(function(){ // new timeout
      alarmUsers.get(username).currentSpam = setInterval(function() { // start spam 
   ...
   setAlarm: function(data) {
      if (data.msg.startsWith("!alarm ")) {
         ...
         if (ms >= 1000 && ms <= 604800000) {
            ...
            setAlarmHelper(data.username, ms, days, hours, mins, seconds); 
         } else {
