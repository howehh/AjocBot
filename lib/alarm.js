...
const alarmJson = require('./../data/alarms');
...
// Sets alarms of people who had alarms saved in file
for (let i = 0; alarmJson.users !== undefined && i < alarmJson.users.length; i++) {
   setTimer(alarmJson.users[i].name, alarmJson.users[i].remainingMs, alarmJson.users[i].originalMs);
}
...
function setAlarmHelper(username, ms, isChatMsg) {
   let timeLeft = getFormattedTime(ms);
   const msg = `${username}: I will spam whisper you in (${timeLeft}) AlizeeOP`;
   ...
   alarmData.alarmUsers.set(username, {
      "timeout": setTimeout(function(){
         ...
         alarmData.alarmUsers.get(username).spam = setInterval(function() {
            let originalTime = getFormattedTime(originalMs);
            bot.pm(username, `YOUR ALARM FOR (${originalTime}) IS OVER AlizeeREE`
   ...
   setAlarm: function(data) {
      if (data.msg.trim() === "!alarm") {
         if (!alarmData.alarmUsers.has(data.username)) {
            ...
            const timeLeft = getFormattedTime(getTimeLeft(data.username));
            bot.chat(`${data.username}: you have (${timeLeft}) left on your alarm`); 
         }
