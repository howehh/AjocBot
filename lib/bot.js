...
const botData = {
   ...
   firestoreDb: new Firestore({
     projectId: 'scientific-pad-265302',
     keyFilename: 'firestore.json',
   }),
...
module.exports = {
   ...
   firestoreDb: function() {
      return botData.firestoreDb;
   },
   ...
   setMotd: function(message) {
      // Cytube kicks bot for no good reason if it doesn't have permissions to set motd
      if (this.isMod(this.BOTNAME)) {
         this.socket().emit("setMotd", {
            motd: message
         });
         return true;
      }
