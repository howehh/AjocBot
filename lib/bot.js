...
socketConnect.on('emoteList', function(emotes) {
	for (let i = 0; i < emotes.length; i++) {
		data.emoteNames.push(emotes[i].name.toLowerCase());
   }
});

// When an emote is added, the name is added to emoteNames array
socketConnect.on('updateEmote', function(emote) {
   if (!data.emoteNames.includes(emote["name"].toLowerCase())) {
		data.emoteNames.push(emote["name"].toLowerCase());
	}
});
...
socketConnect.on('removeEmote', function(emote) {
   let emoteIndex = data.emoteNames.indexOf(emote["name"].toLowerCase());
   if (emoteIndex !== -1) {
      data.emoteNames.splice(emoteIndex, 1);
   }
});
...
function setUserData(data) {
   data.userData.set(data.name, data.rank);
}
...
module.exports = {
   ...
   isAdmin: function(username) {
      return data.userData.has(username) && data.userData.get(username) >= 5;
   },
   ...
   cooldown: function(funcToPause, duration) {
      ...
      if (!data.cooldowns.has(funcToPause) || data.cooldowns.get(funcToPause) === true) {
         data.cooldowns.set(funcToPause, false); 
         setTimeout(function() {
            data.cooldowns.set(funcToPause, true);
         }, duration);
         return true;
