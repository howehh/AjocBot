...
// Calculates the appropriate sound price for a person. Works on a somewhat marginal basis:
// the price is a percentage of the user's points, but will be no lower than 25 and no 
// higher than 100
function calculateSoundPrice(username) {
   const minPrice = 25;
   const maxPrice = 100;
   if (points.hasPoints(username)) {
      let price = Math.round(store.SOUND_RATE * points.getPoints(username));
...
const chatFunctions = {
   ...
   store: function(data) {
      if (data.msg.indexOf("!store") !== -1) {
         bot.chat("*Buy things with your points:* /br/" +
                  (store.SOUND_RATE * 100) + "% of pts (min 25 / max 100): Play a sound `!sound`/br/" +
      ...
      if (data.msg.indexOf("!sound") !== -1) {
         ...
         axios.get("https://pastebin.com/raw/JnLq6m7K").then(function(res) { // sounds from pastebin
            ...
            if (data.msg.trim() === "!sound" || data.msg.trim() === "!sounds") { // A help command
               ...
               const price = store.testing ? 0 : calculateSoundPrice(username);
   ...
   leader: function(data) {
      if (data.msg.indexOf("!leader") !== -1) {
         ...
         if (store.testing || (points.hasPoints(username) && points.getPoints(username) >= price)) {
