...
// Gets the list of secret emote names
// json format: {"secretEmotes": [ "abc", "def" ]}
function getSecretEmotes() {
   storeData.secretEmotes.splice(0, storeData.secretEmotes.length);
   bot.firestoreDb().collection('pastes').doc('secret').get().then(doc => {
      let json = JSON.parse(doc.data().json);
      if (json.secretEmotes !== undefined) {
         storeData.secretEmotes.push.apply(storeData.secretEmotes, json.secretEmotes);
      }
   });
}
getSecretEmotes();
...
const chatEvents = {
   ...
   sound: function(data) {
      if (data.tokens[0] === "!sound" || data.tokens[0] === "!sounds") {
         callbacks.cooldown(chatEvents.sound, 3000);
         const username = data.username.toLowerCase();
         bot.firestoreDb().collection('pastes').doc('sounds').get().then(doc => {
            let json = JSON.parse(doc.data().json);
            const soundNames = [];
            soundNames.push.apply(soundNames, json.sounds);
...
const chatAndWhispers = { // special case: can be called in chat as well
   ...
   secretEmote: function(data) {
      if (data.msg.startsWith("!secretemote")) {
         ...
         if (!storeData.buyers.has(username)) {
            ...
            bot.pm(username, "You are already purchasing a secret emote AlizeeWeird");
         }
      }
   }
}
callbacks.addChatEvents(chatEvents);
callbacks.addChatAndWhispers(chatAndWhispers);