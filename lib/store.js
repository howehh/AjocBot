...
const chatFunctions = {
   ...
   sound: function(data) {
      if (data.msg.indexOf("!sound") !== -1) {
         ...
         axios.get("https://pastebin.com/raw/JnLq6m7K").then(function(response) {
            // get list of sounds from a pastebin
            const soundNames = [];
            soundNames.push.apply(soundNames, response.data.sounds);
