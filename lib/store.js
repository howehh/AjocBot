...
function addBuyer(username) {
   ...
   storeData.buyers.set(username, {"purchasing": true, "choice": -1, "confirmTimeout": null});
}
...
function resetBuyerStatus(username) {
   clearTimeout(storeData.buyers.get(username.toLowerCase()).confirmTimeout);
   storeData.buyers.get(username.toLowerCase()).purchasing = false;
   storeData.buyers.get(username.toLowerCase()).choice = -1;
}
...
function setConfirmTimeout(username) {
   clearTimeout(storeData.buyers.get(username).confirmTimeout);
   
   storeData.buyers.get(username).confirmTimeout = setTimeout(function() {
...
const chatFunctions = {
   ...
   store: function(data) {
      if (data.msg.indexOf("!store") !== -1) {
         bot.chat("*Redeem stuff with points:* /br/" +
   ...
   leader: function(data) {
      if (data.msg.indexOf("!leader") !== -1) {
         ...
         if (points.hasPoints(username) && points.getPoints(username) >= price) {
            bot.chat(username + ": you spent " + price + " pts to have leader permissions"
            ...
            bot.assignLeader(data.username);
            setTimeout(function() {
               bot.assignLeader("");
            }, 60000);
            points.adjustPoints(username, -(price));
         } else {
            bot.chat(username + ": you do not have enough points (need " + price + ")");
         }
      }
   },
...
const pmFunctions = {
   ...
   buySecretEmote: function(data) {
      const username = data.username.toLowerCase();
      if (storeData.buyers.has(username) && storeData.buyers.get(username).purchasing) { 
         ...
         if (!isNaN(choice) && choice > 0 && choice <= storeData.secretEmotes.length) {
            ...
            bot.pm(username, "You are purchasing " + storeData.secretEmotes[choice - 1] + " for " +
...
callbacks.addChatEvents(chatFunctions);
callbacks.addWhisperEvents(pmFunctions);
callbacks.addChatAndWhispers(chatAndWhisperFunctions); // special case: can be called in chat as well