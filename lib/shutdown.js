...
function saveData(success, fail) {
   if (canSave()) {
      ...
      shutdownCallbacks.data.forEach(function(func) {
         let dataObj = func();
         // save locally
         fs.writeFileSync(path.join(__dirname, './../data/' + dataObj.filename), dataObj.data);
...
// Safeguard: every 30 min, save data
...
module.exports = {
   ...
   // Adds a callback function that returns a data object that will be used to
   // create a Gist options object to the data array
   //
   // data = {
   //   "filename": "FILE_NAME",
   //   "data": SOME_DATA
